...
export class ImageGrid<T>{
    ...
    customEdgeBehaviour: (p: Point) => Point = (p) => p;
    ...
    getIndex(x: number, y: number){
        ...
        else if(this.edgeBehaviour == EdgeBehaviour.Mirror){
            let tileX = Math.floor(x / this.width);
            let tileY = Math.floor(y / this.height);
        ...
        else if(this.edgeBehaviour == EdgeBehaviour.Custom){
            let o = this.customEdgeBehaviour({x, y});
